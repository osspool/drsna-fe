...
import { generateTreatmentOGImage, generateOGImageURL, generateResourceOGImage } from './og-helpers';
...
export async function buildMetadata(
  ...
  // Generate dynamic OG image based on entity type
  let ogImageUrl;

  // 1. Treatment pages (have categoryId + quickStats)
  if (entity.categoryId && entity.quickStats) {
    ogImageUrl = generateTreatmentOGImage(entity);
  }
  // 2. Resource guides (have readTime/estimatedRead or sections)
  else if (entity.readTime || entity.estimatedRead || (entity.sections && Array.isArray(entity.sections))) {
    ogImageUrl = generateResourceOGImage({
      title: entity.title,
      description: entity.subtitle || metaDescription?.substring(0, 100) || '',
      category: entity.category,
    });
  }
  // 3. Generic fallback (categories, about, etc.)
