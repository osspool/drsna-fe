...
export const blockSchemas = {
  'hero.category': ['title'],
  'hero.treatment': [], // TreatmentHero uses treatment.hero.headline || treatment.title as fallback
...
export function validateBlockProps(blockId, props, requiredOverride) {
  ...
  const missing = requiredFields.filter(field => {
    // Check at props level first (for 'data', 'variant', etc.)
    if (props[field] !== undefined) return false;
    // Then check inside props.data (for nested fields like 'title', 'features')
    if (props.data && props.data[field] !== undefined) return false;
    return true;
  });
